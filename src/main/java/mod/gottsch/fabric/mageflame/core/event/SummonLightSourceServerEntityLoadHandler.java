 ...
 *
 */
public class SummonLightSourceServerEntityLoadHandler implements ServerEntityEvents.Load {
    ...
    public void onLoad(Entity entity, ServerWorld world) {
        ...
        if (entity instanceof ISummonedLightSourceEntity lightSourceEntity) {
            ...
            if (lightSourceEntity.getOwnerUUID() != null) {
                ...
                if (!SummonFlameRegistry.isRegistered(lightSourceEntity.getOwnerUUID())) {
                    // MageFlame.LOGGER.debug("owner is NOT registered -> {}", flameEntity.getOwnerUUID().toString());
                    SummonFlameRegistry.register(lightSourceEntity.getOwnerUUID(), entity.getUuid());
                }
                /*
                 * NOTE this is an edge-case scenario where the entity was not unregistered and killed
                 *  and it attempts to reunite with owner.
                 */
                else if (!Objects.equals(SummonFlameRegistry.get(lightSourceEntity.getOwnerUUID()), entity.getUuid())) {
                     ...
                     */
                    Entity existingEntity = world.getEntity(SummonFlameRegistry.get(lightSourceEntity.getOwnerUUID()));
